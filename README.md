...
3. **Configure Environment Variables**
   ...
   **Note:** Admin authentication is securely handled by Firebase Auth. Create your initial admin users directly in the Firebase Console matching the emails defined in your `.env`.
   Firebase Admin credentials are recommended for server-side complaint submission. Without them, local development can still use the current anonymous Firestore create rules, but production should use the Admin SDK path.
...
5. **Run Milestone Smoke Checks**
   ```bash
   npm run test:milestone1
   ```
   This verifies the QR reporting link and baseline Firebase rule guards before deeper security refactors.

        ...
        <div className="container-x max-w-[760px]">
          ...
          <Reveal delay={160}>
            <p className="mt-5 text-lg text-[color:var(--text-secondary)] leading-[1.7]">
              A lightweight, browser-first stack that runs anywhere a phone has signal —
              and degrades gracefully when it doesn't.
            </p>
          </Reveal>
        </div>
      </section>
  ...
  return (
    <section className="pb-20">
      <div className="container-x">
        <div className="flex gap-6 border-b" style={{ borderColor: "var(--border)" }} role="tablist">
          {tabs.map((t, i) => (
            <button
              key={t}
              role="tab"
            ...
            >
              {t}
              {tab === i && (
                <span className="absolute inset-x-0 -bottom-px h-0.5" style={{ background: "var(--primary)" }} />
              )}
            </button>
          ))}
        </div>
...
function SystemFlow() {
  const nodes = ["QR code", "Mobile browser", "Geolocation + form", "Secure cloud", "Admin dashboard"];
  return (
    <div className="bg-white rounded-[16px] p-6 lg:p-10 overflow-x-auto" style={{ border: "1px solid var(--border)" }}>
      <div className="flex items-center gap-3 min-w-max">
        {nodes.map((n, i) => (
          <div key={n} className="flex items-center gap-3">
            <div
              className="px-4 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap"
              style={{ background: "var(--primary-tint)", color: "var(--primary)", border: "1px solid rgba(27,79,216,0.18)" }}
            >
              {n}
            </div>
            {i < nodes.length - 1 && (
...
function Security() {
  ...
  return (
    <div className="grid sm:grid-cols-3 gap-5">
      {items.map((it) => (
        <div key={it.title} className="p-6 bg-white rounded-[16px]" style={{ border: "1px solid var(--border)" }}>
          <div className="w-11 h-11 rounded-[10px] flex items-center justify-center" style={{ background: "var(--primary-tint)" }}>
            <it.icon size={22} className="text-[color:var(--primary)]" />
          </div>
          <h3 className="mt-4 text-lg font-bold">{it.title}</h3>
          <p className="mt-2 text-sm text-[color:var(--text-secondary)] leading-[1.65]">{it.body}</p>
        </div>
      ))}
    </div>
  );
    ...
    <section className="section-y" style={{ background: "var(--surface-2)" }}>
      <div className="container-x">
        ...
        <div ref={ref} className="mt-14 relative">
          <div className="absolute left-5 lg:left-1/2 lg:-translate-x-1/2 top-0 bottom-0 w-[2px]" style={{ background: "var(--border)" }} aria-hidden="true">
            <div
          ...
          <ol className="space-y-10">
            {weeks.map((w, i) => (
              <Reveal key={w.n} delay={i * 60}>
                <li className={`relative grid lg:grid-cols-2 gap-4 lg:gap-12 items-center ${i % 2 ? "lg:[&>div:first-child]:order-2" : ""}`}>
                  <span
                    aria-hidden="true"
                  ...
                  <div className={i % 2 ? "lg:pl-16 pl-16 lg:pr-0" : "pl-16 lg:pl-0 lg:pr-16 lg:text-right"} />
                  <div className={i % 2 ? "lg:pr-16 pl-16 lg:pl-0" : "pl-16 lg:pl-16"}>
                    <div className="bg-white rounded-[16px] p-5" style={{ border: "1px solid var(--border)", boxShadow: "var(--shadow-sm)" }}>
                      <div className="text-xs font-bold text-[color:var(--primary)]" style={{ letterSpacing: "0.08em" }}>
                        WEEK {w.n}
                      </div>
                      <div className="mt-1 text-lg font-bold">{w.phase}</div>
                      <p className="mt-1.5 text-sm text-[color:var(--text-secondary)]">{w.activity}</p>
                      <span

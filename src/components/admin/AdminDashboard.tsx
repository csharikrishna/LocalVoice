...
import { collection, query, orderBy, updateDoc, doc, getDocs, getDoc, deleteDoc, writeBatch } from "firebase/firestore";
...
import { Loader2, MapPin, Lock, LogOut, Download, ArrowUpDown, Search, Filter, X, AlertCircle, Wrench, CheckCircle2, BarChart3, RefreshCw, ChevronDown, Check, Navigation, Trash2, Edit3, Save, Flame, CheckSquare } from "lucide-react";
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
export function AdminDashboard({ role, department, squadId }: { role: AdminRole; department?: string | null; squadId?: string | null }) {
  ...
  const [activeTab, setActiveTab] = useState<'tickets' | 'staff'>('tickets');
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedIds.length} selected issues?`)) return;
    ...
    try {
      ...
      toast.error("Failed to delete selected issues: " + err.message);
    } finally {
  ...
  const domainComplaints = useMemo(() => {
    return complaints.filter(c => {
      ...
      if ((role === "department_admin" || role === "field_worker") && department && c.department !== department) return false;
      return true;
    });
  ...
  const handleStatusChange = useCallback(async (id: string, newStatus: string) => {
    ...
    try {
      ...
      toast.success(`Status updated to "${newStatus === "open" ? "Open" : newStatus === "working" ? "In Progress" : "Closed"}"`);
    } catch (err) {
  ...
  ], [handleStatusChange, handleDepartmentChange, handleImageClick, editingId, editingDescription, role, isSelectMode, handleEditDescription, handleDelete]);
  ...
  return (
    <div className={role === "department_admin" ? "px-4 max-w-7xl mx-auto flex flex-col pb-12 w-full mt-6" : "min-h-screen pt-20 px-4 max-w-7xl mx-auto flex flex-col pb-12"}>
        ...
        <div className="flex items-center gap-4 border-b border-gray-200 mb-6">
          <button
            ...
            className={`pb-3 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'tickets' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
            }`}
          >
            Ticket Dispatch
          </button>
          <button
            ...
            className={`pb-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${
              activeTab === 'staff' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
            }`}
          >
            Staff Management
          </button>

...
export function AnalyticsTab({ complaints, slaEnabled, setSlaEnabled }: AnalyticsTabProps) {
  const stats = useMemo(() => {
    ...
    complaints.forEach((c) => {
      ...
      // Time parsing
      const createdAt = c.timestamp?.toDate ? c.timestamp.toDate() : new Date(c.timestamp);
    ...
    return {
      ...
      slaBreachCount,
      departmentData: Object.entries(byDepartment).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count),
      categoryData: Object.entries(byCategory).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count),
      trendData: Object.entries(byMonth).map(([name, count]) => ({ name, count })), // Not sorted to keep chronological (roughly)
    };
  }, [complaints]);
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="w-full flex flex-col gap-6">
      ...
      {/* Header & Controls */}
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/60 backdrop-blur-xl p-4 rounded-2xl border border-white/50 shadow-sm">
        <div>
          <h2 className="text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">System Analytics</h2>
        ...
        <div className="flex items-center gap-4 bg-white/80 px-4 py-2 rounded-xl border border-gray-100 shadow-inner w-full sm:w-auto justify-between sm:justify-end">
          <div className="flex flex-col text-right">
            ...
            <span className="text-[10px] text-gray-500">{slaEnabled ? 'Active on dashboard' : 'Hidden'}</span>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-4 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow">
        ...
        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-4 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow">
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-gray-900 tracking-tight">{stats.avgResolutionHours}<span className="text-lg text-gray-500 font-normal">h</span></h3>
          </div>
        </motion.div>

        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex items-center gap-4 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow">
          <div className={`p-3.5 text-white rounded-xl shadow-lg ${stats.slaBreachCount > 0 ? "bg-gradient-to-br from-rose-500 to-red-600 shadow-red-500/20" : "bg-gradient-to-br from-gray-400 to-gray-500 shadow-gray-500/20"}`}>
            <AlertTriangle size={24} />
          </div>
          <div>
            <p className="text-sm text-gray-500 font-medium">SLA Breach Rate</p>
            <h3 className="text-3xl font-bold text-gray-900 tracking-tight">{stats.slaBreachRate}<span className="text-lg text-gray-500 font-normal">%</span></h3>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
          <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2"><span className="w-2 h-6 bg-blue-500 rounded-full inline-block"></span> Issues by Department</h3>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.departmentData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <RechartsTooltip cursor={{ fill: "rgba(59, 130, 246, 0.05)" }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }} />
        ...
        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
          <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2"><span className="w-2 h-6 bg-purple-500 rounded-full inline-block"></span> Issues by Category</h3>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.categoryData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} strokeOpacity={0.4} />
                <XAxis dataKey="name" tick={{ fontSize: 12, fill: '#6b7280' }} />
                <YAxis />
                <RechartsTooltip cursor={{ fill: "rgba(139, 92, 246, 0.05)" }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }} />
        ...
        <motion.div variants={itemVariants} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] lg:col-span-2">
          <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2"><span className="w-2 h-6 bg-emerald-500 rounded-full inline-block"></span> Reporting Trend (Monthly)</h3>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.trendData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="count" stroke="#10b981" strokeWidth={4} dot={{ r: 5, fill: '#10b981', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 8 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
